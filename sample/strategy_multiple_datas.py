...
class SmaCross1(bt.Strategy):
    ...
    def notify_trade(self, trade):
        self.log(
            "placing trade for {}. target size: {}".format(
                trade.getdataname(), trade.size
            )
        )
...
if __name__ == "__main__":
    ...
    if not IS_BACKTEST:
        data0 = DataFactory(
            dataname=SYMBOL1, historical=False, timeframe=bt.TimeFrame.Days
        )
        data1 = DataFactory(
            dataname=SYMBOL2, historical=False, timeframe=bt.TimeFrame.Days
        )
