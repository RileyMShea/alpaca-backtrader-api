...
class API(tradeapi.REST):
    def _request(self, method, path, data=None, base_url=None, api_version=None):
        ...
        # Added the try block
        try:
            return super()._request(method, path, data, base_url, api_version)
...
class AlpacaStore(metaclass=MetaSingleton):
    ...
    def __init__(
        ...
        self.oapi = API(
            self.p.key_id, self.p.secret_key, self.p.base_url, self.p.api_version
        )
    ...
    def _t_candles(
        ...
        try:
            if self.p.usePolygon:
                cdl = self.get_aggs_from_polygon(
                    dataname, dtbegin, dtend, granularity.value, compression
                )
            else:
    ...
    def get_aggs_from_polygon(self, dataname, dtbegin, dtend, granularity, compression):
        """
        ...
        if granularity == "day":
            cdl = self.oapi.polygon.historic_agg_v2(
                ...
                to=self.iso_date(dtend.isoformat()),
            ).df
        else:
            cdl = pd.DataFrame()
            segment_start = dtbegin
            segment_end = (
                segment_start + timedelta(weeks=2)
                if dtend - dtbegin >= timedelta(weeks=2)
                else dtend
            )
    ...
    def get_aggs_from_alpaca(self, dataname, start, end, granularity, compression):
        """
        ...
        def _iterate_api_calls():
            ...
            while not got_all:
                ...
                r = self.oapi.get_barset(
                    dataname, timeframe, limit=1000, end=curr.isoformat()
                )[dataname]
                if r:
                    ...
                    response = r
                    if earliest_sample <= (
                        pytz.timezone(NY).localize(start)
                        if not start.tzname()
                        else start
                    ):
                        got_all = True
        ...
        if not start:
            response = self.oapi.get_barset(dataname, granularity, limit=1000, end=end)[
                dataname
            ]._raw
        else:
    ...
    def order_create(self, order, stopside=None, takeside=None, **kwargs):
        ...
        okwargs["symbol"] = (
            order.data._name if order.data._name else order.data._dataname
        )
        okwargs["qty"] = abs(int(order.created.size))
        okwargs["side"] = "buy" if order.isbuy() else "sell"
        okwargs["type"] = self._ORDEREXECS[order.exectype]
        okwargs["time_in_force"] = "gtc"
        if order.exectype not in [bt.Order.Market, bt.Order.StopTrail, bt.Order.Stop]:
            ...
            try:
                ...
                try:
                    ...
                    if "code" in o._raw:
                        ...
                        self.put_notification("General error from the Alpaca server")
                    self.broker._reject(oref)
                    continue
    ...
    def _process_transaction(self, oid, trans):
        ...
        if ttype in self._X_ORDER_FILLED:
            ...
            print(f"Process transaction - Order type: {ttype}")
            self.broker._reject(oref)
